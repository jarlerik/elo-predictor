...
const PlayedWinnerBets: React.FC = () => {
  const [betFiles, setBetFiles] = useState<BetFile[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [expandedRows, setExpandedRows] = useState<Set<string>>(new Set());
  const [betData, setBetData] = useState<Record<string, WinnerBet[]>>({});
  ...
  const fetchBetFiles = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch("/api/bets/list-winners");
      if (!response.ok) {
        throw new Error("Failed to fetch winner bet files");
      }
  ...
  const handleAddResult = async () => {
    ...
    try {
      // Extract game name from filename (remove _winner.json extension)
      const gameName = selectedBet.filename.replace("_winner.json", "");
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {getTeamName(selectedBet.bet.team)}
              </span>
              <span
                style={{
                  color: "#a3a3a3",
      ...
      <div className="bets-section">
        {betFiles.length === 0 ? (
          <div className="empty-state">
            <p>No winner bet files found. Save some bets to see them here.</p>
          </div>
        ) : (
...
export default PlayedWinnerBets;
